...
MAIN:
    BTFSS PORTA,4,A  ; Revisa el bit 4 del puerto A usando el acces bank
    GOTO ES_CERO
    GOTO ES_UNO
...
/*
MAIN:
    BTFSS PORTA,2,A	; Revisa el puerto 2 y si es 1 salta a la sig instruc. Revisa el bit 2 de PORTA usando el Acces bank
    BRA ES_CERO ;GOTO ES_CERO
    BRA ES_UNO	;GOTO ES_UNO

ES_CERO:
    MOVLW 00000001B	; 0X01 en hexadecimal
    MOVWF datoA,a   ; Mueve el contenido de w a la variable datoA
    MOVFF datoA,PORTB	; Mueve el contenido de datoA al registro PORTB
    BRA MAIN	;GOTO MAIN
    
ES_UNO: 
    MOVLW 01101010B	; 0X6A en hexa
    MOVWF datoB,a
    MOVFF datoB,PORTB
    BRA MAIN	;GOTO MAIN
